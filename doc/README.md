# Documentação booptube

| Documento | Público | Conteúdo |
|-----------|---------|----------|
| [projeto.md](projeto.md) | **Todos** | Visão geral do projeto, instalação, CLI e GUI, arquitetura |
| [build-windows.md](build-windows.md) | **Quem compila** | Build no Windows (PowerShell) |
| [build-linux.md](build-linux.md) | **Quem compila** | Build no Linux (Makefile) |
| [build-macos.md](build-macos.md) | **Quem compila** | Build no macOS (Makefile) |
| [gui.md](gui.md) | **Usuário / quem compila a GUI** | Instalar, compilar e usar o `booptube-gui` |
| [usuario.md](usuario.md) | **Usuário final** | Como usar CLI e GUI no dia a dia |
| [cli.md](cli.md) | **Desenvolvedor** | Referência técnica, Makefile, config, embed |
| [web.md](web.md) | **Quem faz deploy** | Modo web — Docker, API REST, Grafana, logs |
| [installer.md](installer.md) | **Quem publica releases** | Portable vs bundled, instaladores Windows/Linux/macOS |

## Por onde começar

- Quer **compilar no seu sistema** → escolha o guia de build:
  - [Windows](build-windows.md)
  - [Linux](build-linux.md)
  - [macOS](build-macos.md)
- Quer **usar a interface gráfica** → [gui.md](gui.md)
- Quer **usar o terminal** → [usuario.md](usuario.md) (seção CLI)
- Quer **entender tudo que existe no projeto** → [projeto.md](projeto.md)
- Quer **detalhes técnicos (embed, config, Makefile)** → [cli.md](cli.md)
- Quer **subir o servidor web com Docker** → [web.md](web.md)
- Quer **gerar zip portable ou instalador** → [installer.md](installer.md)
